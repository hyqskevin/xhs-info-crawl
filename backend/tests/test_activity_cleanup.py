...
def test_cleanup_removes_window_outliers_and_rebuilds_exports_without_deleting_sources(db_session, tmp_path):
    ...
    db_session.add(note)
    db_session.flush()
    valid = Activity(note_id=note.id, name="有效活动", city_code="nb", start_time=datetime(2026, 7, 20), location="广场", type="其他", status="RAW")
    expired = Activity(note_id=note.id, name="历史活动", city_code="nb", start_time=datetime(2024, 7, 20), location="广场", type="其他", status="RAW")
