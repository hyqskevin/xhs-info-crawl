...
from app.services.report import generate_note_markdown
...
def test_weekly_report_selects_approved_posts_by_publish_time_and_keeps_all_children(db_session) -> None:
    ...
    entries = select_notes(db_session, GenerateRequest(week="2026-W29", cities=["nb"]))
    ...
    assert "![图片 1](/api/v1/reports/image/images/post.jpg)" in markdown
    assert "图片链接" not in markdown
