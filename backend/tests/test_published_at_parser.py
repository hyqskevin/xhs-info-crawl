...
口径修订 spec: docs/superpowers/specs/2026-07-27-review-consistency-fixes-design.md
（统一返回 Asia/Shanghai 北京墙钟，与雪花路径及库存储口径一致）
"""
...
def test_parse_absolute_iso_date_returns_beijing_wall(now_local: datetime) -> None:
    result = parse_published_at("2025-07-20", now_local=now_local)
    assert result.source == "absolute"
    assert result.confidence == 1.0
    assert result.value == datetime(2025, 7, 20, 0, 0, tzinfo=SHANGHAI)
...
def test_parse_absolute_iso_with_time(now_local: datetime) -> None:
    ...
    assert result.source == "absolute"
    assert result.value == datetime(2025, 7, 20, 18, 30, tzinfo=SHANGHAI)
...
def test_parse_month_day_with_explicit_pair_infers_year(now_local: datetime) -> None:
    ...
    assert result.value == datetime(2026, 7, 20, 18, 0, tzinfo=SHANGHAI)
...
def test_parse_month_day_in_past_year_rolls_back_to_last_year(now_local: datetime) -> None:
    ...
    assert result.source == "month_day"
    assert result.value == datetime(2026, 1, 5, 0, 0, tzinfo=SHANGHAI)
...
def test_parse_month_day_future_rolls_back_to_previous_year(now_local: datetime) -> None:
    ...
    assert result.source == "month_day"
    assert result.value == datetime(2025, 12, 15, 0, 0, tzinfo=SHANGHAI)
...
def test_parse_relative_day(now_local: datetime) -> None:
    ...
    assert result.value == now_local - timedelta(days=2)
...
def test_parse_relative_hour(now_local: datetime) -> None:
    ...
    assert result.value == now_local - timedelta(hours=5)
...
def test_parse_relative_minute(now_local: datetime) -> None:
    ...
    assert result.value == now_local - timedelta(minutes=30)
...
def test_extract_published_at_returns_absolute_from_relative_payload(now_local: datetime) -> None:
    ...
    expected = fallback.astimezone(SHANGHAI) - timedelta(days=1)
