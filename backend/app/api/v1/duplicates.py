...
def candidates(_:User,db:DB,status:str|None=None,page:int=1,page_size:int=20):
    filters=[DuplicateCandidate.status==(status or 'pending')]; total=db.scalar(select(func.count()).select_from(DuplicateCandidate).where(*filters)) or 0; rows=db.scalars(select(DuplicateCandidate).where(*filters).offset((page-1)*page_size).limit(page_size)).all()
...
def merge(candidate_id:int,payload:MergeIn,_:User,db:DB):
    ...
    removed.status='DELETED'; c.status='merged'; c.resolution=f'keep_{payload.keep}'; c.merged_activity_id=kept.id; c.resolved_at=datetime.now(timezone.utc); db.commit()
