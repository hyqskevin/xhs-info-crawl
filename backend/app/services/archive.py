...
def archive_task_result(root: Path, started_at: datetime, task_id: int, note: Note, image_rows: list[tuple[Path, NoteImage]], activities: list[Activity]) -> Path:
    ...
    for item in activities:
        sheet.append([item.name, item.status, item.start_time.isoformat(), item.end_time.isoformat() if item.end_time else "", item.location, item.price, item.type, ",".join(map(str, item.source_image_indexes or [])), item.source_url, item.summary])
