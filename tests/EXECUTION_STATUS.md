...
```text
后端：96 passed, 1 skipped
前端组件/模块：20 passed（11 个测试文件）
前端 Playwright（Google Chrome）：35 passed
生产构建：passed
```
...
| 活动管理 | 禁止手工创建、分页、筛选、详情、审核、软删除、关联笔记与鉴权来源图片、路径边界 | `backend/tests/test_activities_api.py`、`backend/tests/test_activity_batch_delete.py` |
...
| 处理服务 | 去重评分与合并、规则/LLM 提取、模糊日期降级、OCR、标题关键词硬过滤、阶段重试、单笔记隔离、安全停止与续跑 | `backend/tests/test_pipeline_services.py`、`backend/tests/test_multi_activity_archive.py`、`backend/tests/test_crawl_task_resilience.py` |
...
| 前端组件/模块 | App、布局、7 个页面组件、路由守卫、HTTP Token 请求头 | `frontend/src/**/*.spec.ts`，共 11 个文件、20 条场景 |
| 前端浏览器 E2E | 登录与菜单、仪表盘抓取/安全停止/续跑/跳过进度、活动筛选/审核/批量删除/宽版图片详情、去重、单城市周报、配置与 OpenCLI | `frontend/e2e/navigation.spec.ts`、`frontend/e2e/business.spec.ts`、`frontend/e2e/documented-flows.spec.ts` |
