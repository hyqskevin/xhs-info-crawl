...
def test_ensure_env_file_preserves_empty_admin_password(tmp_path: Path):
    """v0.7.0+9:INITIAL_ADMIN_PASSWORD 是用户配置 key,不应进 .app/.env。

    之前逻辑:确保 .app/.env 含 INITIAL_ADMIN_PASSWORD= 留空行(让后端 migration 走默认 Admin@123)
    当前逻辑:用户配置 key 全部由 DATA_DIR/.env 承载,.app/.env 只放 launcher 系统 key
    INITIAL_ADMIN_PASSWORD 应去 DATA_DIR/.env 走(由 ensure_data_dir_env 复制)
    ...
    # v0.7.0+9:用户配置 key 不进 .app/.env
    assert "INITIAL_ADMIN_PASSWORD" not in content, (
        "INITIAL_ADMIN_PASSWORD 是用户配置 key,不应进 .app/.env;应去 DATA_DIR/.env"
    )
...
def test_ensure_env_file_generates_secret_key_if_placeholder(tmp_path: Path):
    """v0.7.0+9:ensure_env_file 不再在 .app/.env 写 SECRET_KEY。

    老逻辑: .env.example 里的 SECRET_KEY 占位值 → 替换为随机密钥写 .app/.env
    新逻辑: SECRET_KEY 是用户配置 key,不应该在 .app/.env(走 DATA_DIR/.env)
    ...
    content = env_path.read_text()
    # 占位值不在 content(因为 SECRET_KEY 行被过滤掉)
    assert "replace-with-a-random-local-secret" not in content
    # 整个 .app/.env 不应再有 SECRET_KEY 这一行
    assert "SECRET_KEY=" not in content, (
        "v0.7.0+9:SECRET_KEY 是用户配置 key,不应进 .app/.env;走 DATA_DIR/.env"
    )
...
def test_build_api_base_url_from_env(tmp_path: Path):
    """build_api_base_url:从 .env 读 API_HOST/API_PORT 拼出 http://host:port。"""
    from launcher.env_bootstrap import build_api_base_url
    env_path = tmp_path / ".env"
    env_path.write_text("API_HOST=127.0.0.1\nAPI_PORT=8001\n")

    assert build_api_base_url(env_path) == "http://127.0.0.1:8001"


def test_build_api_base_url_defaults(tmp_path: Path):
    """build_api_base_url:缺少 key 时用默认 127.0.0.1:8000。"""
    from launcher.env_bootstrap import build_api_base_url
    env_path = tmp_path / ".env"
    env_path.write_text("FOO=bar\n")

    assert build_api_base_url(env_path) == "http://127.0.0.1:8000"


def test_build_cors_origins_from_env(tmp_path: Path):
    """build_cors_origins:从 .env 读 WEB_PORT 加入默认 localhost 列表,生成 cors 列表。"""
    from launcher.env_bootstrap import build_cors_origins
    env_path = tmp_path / ".env"
    env_path.write_text("WEB_PORT=5174\nAPI_HOST=127.0.0.1\n")

    origins = build_cors_origins(env_path)
    assert "http://127.0.0.1:5173" in origins
    assert "http://127.0.0.1:5174" in origins
    assert "http://localhost:5173" in origins


def test_build_cors_origins_includes_web_port(tmp_path: Path):
    """build_cors_origins:WEB_PORT 实际端口必须出现在列表里。"""
    from launcher.env_bootstrap import build_cors_origins
    env_path = tmp_path / ".env"
    env_path.write_text("WEB_PORT=5175\nAPI_HOST=127.0.0.1\n")

    origins = build_cors_origins(env_path)
    assert any(":5175" in o for o in origins)
