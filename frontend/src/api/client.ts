...
export const api={
  ...
  activities:(params={})=>http.get('/activities',{params}),activity:(id:number)=>http.get(`/activities/${id}`),activityImage:(activityId:number,imageId:number)=>http.get(`/activities/${activityId}/images/${imageId}`,{responseType:'blob'}),updateActivity:(id:number,data:object)=>http.put(`/activities/${id}`,data),deleteActivity:(id:number)=>http.delete(`/activities/${id}`),deleteActivities:(ids:number[])=>http.delete('/activities/batch',{data:{ids}}),approveActivities:(ids:number[])=>http.post('/activities/batch/approve',{ids}),
