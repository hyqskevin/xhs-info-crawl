  ...
  <ElCard shadow="never">
    <ElTable :data="rows">
      <ElTableColumn label="推文 A" min-width="220">
        <template #default="scope"><strong>{{ scope.row.note_a?.title }}</strong><div>{{ scope.row.note_a?.published_at ? formatCnDateTime(scope.row.note_a.published_at) : '发布时间待确认' }}</div><div>识别活动 {{ scope.row.note_a?.activity_count || 0 }} 条</div></template>
      </ElTableColumn>
      <ElTableColumn label="推文 B" min-width="220">
        <template #default="scope"><strong>{{ scope.row.note_b?.title }}</strong><div>{{ scope.row.note_b?.published_at ? formatCnDateTime(scope.row.note_b.published_at) : '发布时间待确认' }}</div><div>识别活动 {{ scope.row.note_b?.activity_count || 0 }} 条</div></template>
      </ElTableColumn>
