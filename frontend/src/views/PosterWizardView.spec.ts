...
describe('PosterWizardView', () => {
  ...
  it('step 5 no longer renders hidden preview image (dead code removed)', async () => {
    const wrapper = factory()
    await flushPromises()
    ;(wrapper.vm as any).selectedIds = [1]
    ;(wrapper.vm as any).templateId = 7
    ;(wrapper.vm as any).items = [{ type: 'note', id: 1, title: 'x', fields: {}, image_url: '/img/1.jpg' }]
    ;(wrapper.vm as any).step = 5
    await nextTick()
    const html = wrapper.html()
    expect(html).not.toContain('v-show="false"')
    expect(html).not.toContain('ElImage')
  })
