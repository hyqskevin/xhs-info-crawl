...
async function submit(){if(!username.value.trim()||!password.value){ElMessage.warning('请输入用户名和密码');return}loading.value=true;try{const r=await api.login(username.value,password.value);localStorage.setItem('token',r.data.data.access_token);await router.push('/dashboard')}catch{ElMessage.error('用户名或密码错误')}finally{loading.value=false}}
</script>
