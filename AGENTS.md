...
**所有写操作（文件下载、缓存、临时文件、日志、数据库）必须限制在项目根目录内或系统临时目录内，不得污染项目外部的隐藏配置目录（`~/.paddlex`、`~/.cache` 等）和 `$HOME` 下的其他位置。**

### 硬约束

- 生产代码禁止硬编码 `Path.home()`、`expanduser('~')`、`$HOME` 下的隐藏目录（`~/.paddlex`、`~/.cache`、`~/.config` 等）
- `/tmp/`、`tempfile.gettempdir()` **允许直接使用**（2026-08-24 用户确认放开）—— 用于 Chrome CDP 实测的 user-data-dir、临时下载缓存等场景；用户机器 macOS 自带清理机制（`periodic daily/weekly`）+ 容器 CI 通常每次新启，无残留风险
- 第三方库缓存目录仍必须通过环境变量重定向到项目内（这些库**默认会写 `~/.xxx`**，不重定向会污染 `$HOME` 隐藏目录）：
...
- 测试代码用 pytest `tmp_path` fixture 优先；不直接写 `/tmp`（避免污染其他工具的 `/tmp` 视图，但 `/tmp` 不再被静态扫描 ban）

### 验证

`backend/tests/test_project_internal_writes.py` 静态扫描生产代码，确保无 `Path.home()` / `expanduser('~')` / `~/.paddlex` / `~/.cache` / `~/.config` 等隐藏目录硬编码。**`/tmp` 和 `tempfile.gettempdir()` 不再触发静态扫描报警**（2026-08-24 放开后）。新增服务代码后跑该测试确认合规。
