...
把数据库执行权检查传入 `OpenCLIAdapter`。每条受控 OpenCLI 命令在启动前、启动并登记 PID 后、子进程退出后各检查一次：

- 启动前检查阻止已知停止状态创建新进程；
- 登记后的第二次检查覆盖“检查通过后、进程登记前”发生停止的竞态；
- 第二次检查失败时，adapter 立即结束刚启动的子进程、等待回收并原样抛出停止或执行权失效异常；
- 退出后的第三次检查覆盖 stop API 从另一进程结束子进程的情况，使 `-9` 等退出码优先解释为正常停止，而不是 `OpenCLIError`；
- API 与 adapter 两侧任一方观察到停止，都能让当前命令退出。

stop API 必须先把 `STOP_REQUESTED` 提交到数据库，再结束已登记子进程。这样 worker 从 `communicate()` 返回后一定能读到停止状态，不会在“子进程已结束、停止状态尚未提交”的窗口内把任务误写成 `FAILED`。已经没有 worker 执行的 `FAILED`、`PAUSED` 任务点击结束时直接进入 `STOPPED`，不得停留在无人确认的 `STOP_REQUESTED`。
...
- stop API 先提交 `STOP_REQUESTED` 再结束进程：退出后的检查把被结束进程识别为正常停止；
...
10. 子进程被 stop API 从外部结束后，停止异常优先于非零退出码；
11. stop API 在调用 PID kill 前已经提交 `STOP_REQUESTED`，`FAILED`、`PAUSED` 停止时直接进入 `STOPPED`。
