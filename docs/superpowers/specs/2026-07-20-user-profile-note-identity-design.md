...
- 红测确认 `user/profile` 笔记 URL 原本返回 `None`，导致已处理笔记越过复用检查；纯博主主页负例保持正确。
- 新增严格路径规则后，身份与任务相关回归 `30 passed`。
- 全量：后端 `215 passed, 1 skipped`，前端组件 `28 passed`，Playwright E2E `38 passed`，`git diff --check` 通过。
- 只读核对任务 #7 的真实数据库记录：记录存在、状态为 `PROCESSED`，新规则提取结果与已存 `platform_note_id` 一致；核对过程未输出签名 URL 或 token。
