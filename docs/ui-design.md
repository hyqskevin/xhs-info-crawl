# 页面 UI 交互设计

## 安全验证暂停

任务进入 `PAUSED` 且错误为小红书安全验证时，仪表盘使用 Element Plus `ElAlert` 展示原因，并复用“打开小红书登录”“检测登录并继续”两个按钮。不得使用 emoji，不得自动填写或提交验证码。

## UI 实现规范

- 前端基于 Vue 3，统一使用 **Element Plus**（Element UI 的 Vue 3 对应版本）作为 UI 组件库。
- 图标统一使用 **`@element-plus/icons-vue`**，优先选择语义匹配的 Element Plus 官方图标。
- 禁止使用 Emoji 充当菜单、按钮、状态、提示或装饰图标；无合适图标时优先使用文字，不得用 Emoji 替代。
- 页面布局、表单、按钮、表格、分页、标签、提示、弹窗、抽屉、加载、空状态、上传等能力，优先组合 Element Plus 官方组件实现。
- 尽量不自行实现已有的通用 UI 组件，不重复造轮子。只有 Element Plus 明确不具备所需能力，且业务确有必要时，才允许创建自定义组件。
- 自定义组件必须复用 Element Plus 的颜色、字号、间距、圆角和交互状态，避免形成第二套视觉体系。
- 所有危险操作使用 Element Plus 的确认组件；成功、警告和错误反馈使用 `ElMessage`、`ElNotification` 或 `ElAlert`，不得自行绘制临时提示层。
- 图标按钮必须提供可访问名称或 Tooltip，不能只依赖图标表达含义。

### 表格操作列规范

- 所有列表页的操作列统一加 `class-name="action-column"` 并使用 `min-width` 而非 `width`。
- 全局样式在 `frontend/src/assets/styles/table-actions.css`：
  - 按钮文字不换行（`white-space: nowrap`）。
  - 移动端 ≤768px 时按钮纵向堆叠、占满宽度。
- 按钮数 ≥ 4 时（如 DuplicatesView）前两个按钮直显，其余收入 `ElDropdown` "更多 ▾" 菜单。

### 优先使用的组件映射

| 场景 | Element Plus 组件 |
|------|-------------------|
| 页面框架与导航 | `ElContainer`、`ElAside`、`ElHeader`、`ElMain`、`ElMenu` |
| 列表与筛选 | `ElTable`、`ElForm`、`ElInput`、`ElSelect`、`ElDatePicker`、`ElPagination` |
| 状态展示 | `ElTag`、`ElBadge`、`ElProgress`、`ElResult`、`ElEmpty`、`ElSkeleton` |
| 编辑与详情 | `ElDialog`、`ElDrawer`、`ElDescriptions`、`ElTabs`、`ElImage` |
| 操作与反馈 | `ElButton`、`ElPopconfirm`、`ElMessageBox`、`ElMessage`、`ElNotification` |
| 配置录入 | `ElSwitch`、`ElInputNumber`、`ElUpload`、`ElTransfer` |

若设计稿与上述规范冲突，以复用 Element Plus 官方组件和图标为优先原则。

## 页面清单

| 页面 | 路径 | 主要功能 | 角色 |
|------|------|----------|------|
| 登录页 | `/login` | 账号密码登录 | 所有用户 |
| 仪表盘 | `/dashboard` | 发起抓取、本周数据概览、服务状态 | 管理员/运营 |
| 活动列表 | `/activities` | 查看、搜索、编辑、删除活动 | 管理员/运营 |
| 活动详情 | `/activities/:id` | 编辑单个活动字段 | 管理员/运营 |
| 去重审核 | `/duplicates` | 确认/合并/忽略去重候选 | 管理员/运营 |
| 配置中心 | `/settings` | 城市、关键词、博主白名单、OpenCLI | 管理员 |
| 任务日志 | `/tasks` | 查看历史任务与错误 | 管理员 |
| 周报管理 | `/reports` | 预览、下载、重新生成周报 | 管理员/运营 |

## 仪表盘

- 顶部“发起抓取”卡片：城市单选，关键词多选，时间范围单选，博主多选。
- 所有选项来自配置中心；切换城市后仅显示该城市的关键词和博主，时间默认使用城市配置并允许本次覆盖。
- 至少选择一个关键词或博主才可提交；任务启动前检查 Chrome 小红书登录态。
- 下方“最近抓取任务”卡片每 3 秒刷新，展示发现、下载、OCR、提取、失败、已跳过数量、当前阶段、当前笔记和总进度。
- 等待中或运行中的任务显示“停止抓取”；确认后安全完成当前笔记再停止。失败或已停止任务显示“继续抓取”。
- 顶部卡片：本周抓取笔记数、生成活动数、待审核去重、最近任务状态
- 中部图表：最近 4 周活动数量趋势（按城市）
- 底部列表：最近 5 条任务日志，点击跳转详情

## 活动列表

- 筛选条件：城市、活动类型、举办时间、状态（已审核/待审核/已忽略）
- 表格字段：活动名称、城市、举办时间、地点、费用、类型、来源笔记、状态、操作
- 操作按钮：编辑、删除、忽略、加入周报
- 分页：每页 20 条

## 活动编辑弹窗

- 活动只由抓取任务生成，不提供手工新增入口。
- 筛选：城市名称下拉、活动开始日期范围、中文审核状态。
- 分页：每页 10/20/50/100 条，显示总数和页码跳转。
- 表格支持勾选当前页活动并批量删除；操作前二次确认，成功后刷新列表。
- 编辑审核表单：活动名称、城市、举办时间、地点、费用、活动类型、来源链接、摘要、审核状态。
- 原始笔记预览：标题、正文、来源链接
- 图片列表：缩略图 + OCR 文字
- 操作：保存、取消、标记为已审核
- 活动详情使用 Element Plus 抽屉，桌面端宽度 70%、窄屏 95%。详情表格增加原文标题和原文链接。
- 详情表格下显示“来源页面图片”网格，使用 `ElImage` 懒加载与多图预览；无图片使用 `ElEmpty`。

## 去重审核

- 双栏对比：左侧活动 A，右侧活动 B
- 显示相似度得分、匹配字段高亮
- 操作：合并（保留 A / 保留 B / 合并为新记录）、不是重复、稍后处理
- 保留 A/B 后候选立即从待审核列表移除；未保留活动软删除，保留活动继续保持原审核状态。

## 任务日志

- 仅提供抓取任务监控，不包含城市、关键词等配置输入，也不提供“开始抓取”按钮。
- 列表显示任务 ID、中文状态、发现笔记、成功/失败数量、创建时间和错误。
- 点击“日志”查看任务执行时间线；新的抓取统一从仪表盘发起。

## 配置中心

### 城市管理

- 城市为主配置，表格显示城市名称、关键词标签、抓取时间范围和中文启用状态。
- 新增/编辑时一次维护城市名称、多个关键词、抓取时间范围和启用状态。
- 抓取时间范围仅提供小红书原生选项：不限、一天内、一周内、半年内。
- 最近任务为“等待登录”时，使用 Element Plus 按钮和图标显示“打开小红书登录”“检测登录并继续”；两个操作使用独立 Loading，结束后显示 Toast。
- 活动日期为空时列表、编辑弹窗和详情统一显示“待确认”，不伪造当前时间。
- 城市代码由后端自动生成，仅作内部关联键，界面不显示。
- 操作：新增、编辑、删除、启用/禁用。

### 博主白名单

- 表格：博主主页、博主名称、关联城市名称、中文启用状态
- 操作：新增、编辑、删除

### OpenCLI 配置

- 字段：
  - `OPENCLI_CDP_ENDPOINT`：本地 Chrome CDP 端点（如 `http://localhost:9222`）
  - `OPENCLI_PROFILE`：默认 Chrome profile
  - 搜索间隔（秒，默认 10-15）
  - 单关键词抓取数量（默认 50-100）
- 操作：保存、测试连接；测试请求期间在按钮旁显示独立旋转图标，完成后图标消失并通过 Toast 提示结果。

## 周报管理

- 生成条件使用 Element Plus 周选择器和配置中心的已启用城市下拉；阶段一只支持单城市生成与导出。
- 列表：生成时间、城市、活动数量、操作（预览、下载 Excel/Markdown、删除）
- 预览页：按城市分组，按活动类型排序，显示 Markdown 渲染效果
- 下载：同一批审核结果可生成并下载 `.xlsx` 与 `.md` 文件
# 2026-07-20 推文维度界面补充

- “活动管理”首页一行展示一篇推文，列为推文标题、城市、发布时间、识别活动数、审核状态和操作。
- 推文详情使用宽抽屉，按原文信息、子活动表格、来源图片与 OCR 的顺序展示。
- 推文支持批量通过和批量删除；子活动只在详情内编辑或删除。
- 去重审核比较推文 A/B，不再比较同一推文内的多个活动。
- 周报使用周选择器和城市单选，列表同时展示推文数和活动数，并提供 Markdown/Excel 下载。
- 所有控件继续使用 Element Plus 与官方 icon，不使用 emoji 或自行实现基础 UI 控件。
