...
| matched_blogger_username | VARCHAR(64) NULL | 博主用户名快照（避免博主删除后失效） |
| like_count | INT NULL | 点赞数（详情抓取阶段落库；字段名候选 like_count/liked_count/likes） |
| collect_count | INT NULL | 收藏数（字段名候选 collect_count/collected_count/collects） |
| comment_count | INT NULL | 评论数（字段名候选 comment_count/comments） |
...
# 2026-09-07 时间口径约定（单口径，替代 2026-07-27 双口径）

- **所有时间列统一为北京墙钟 naive**（Asia/Shanghai 墙钟数字，不带 tzinfo）：审计字段（`created_at` / `updated_at` / `started_at` / `finished_at` / `deleted_at` / `resolved_at` / `cooldown_until` 等）与业务字段（`notes.published_at`、`activities.start_time` / `end_time`）同口径。写入侧唯一入口 `app/core/timeutil.now_cn()`（models default / api / services / tasks 全部使用）。
- 存量数据由 alembic `0029_unify_timestamps_to_cn` 迁移：审计列 UTC naive +8h / 带 offset 或 `Z` 的行转 Beijing naive；业务列仅归一带 offset 的误存行（前端 `toISOString` 泄漏），naive 行不动。迁移前自动备份 DB 到 `data/backups/pre-0029-*.db`。
- 查询侧构造边界（日期过滤、`week_bounds` 周报边界、dashboard 周起点）一律使用 **naive 北京墙钟**（`now_cn()` / `datetime.fromisocalendar`），不加 tzinfo；SQLite 按墙钟数字字符串比较。
- **API 出口**：`app/core/json_response.CnJsonResponse` 给所有 naive datetime 字符串补 `+08:00` 后缀（JS 在任何机器时区都能解析出正确时刻）；已带 `Z` / offset 的字符串原样保留。
- **显示侧**：前端统一经 `frontend/src/utils/datetime.ts formatCnDateTime` 格式化（仅格式化，无时区换算语义）；表单提交用 `toCnWallString` 生成北京墙钟 naive ISO 字符串（不再 `toISOString`）。
- **协议/系统时间例外**（保持 aware UTC，不属业务口径）：`app/core/security.py` JWT `exp`；`app/services/maintenance.py` 文件 mtime cutoff（`.timestamp()` 系统时间域）。
